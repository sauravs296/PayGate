...
export function slugify(text: string): string {
  return text
    ...
    .replace(/[^a-z0-9\s-]/g, "") // remove all non-alphanumeric chars (emojis, punctuation)
    .trim()
    .replace(/\s+/g, "-") // replace spaces with hyphens
    .replace(/-+/g, "-"); // dedupe multiple hyphens
}

export function NewApiForm({ baseUrl }: { baseUrl: string }) {
  ...
  return (
    <form action={createApiAction} className="space-y-6 bg-zinc-900/50 p-6 rounded-xl border border-zinc-800">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-zinc-500">Auto-generated clean URL slug, stripped of special characters.</p>
      </div>
